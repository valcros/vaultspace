...
BEGIN
  ...
  WHERE oid = pg_catalog.to_regprocedure(
    ...
    AND pg_catalog.pg_get_function_identity_arguments(oid) = 'input_token text'
    ...
    AND pg_catalog.md5(prosrc) = 'bb08b359335ac3b07abbf7cf50c4708d'
  ...
  IF unexpected_acl_rows <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'BOOTSTRAP_SESSION_PREEXISTING_ACL_INVALID';
  END IF;
  ...
  -- migration owns the live session grant.
  ...
  GRANT EXECUTE ON FUNCTION public.bootstrap_session_resolve_v1(text)
    TO vaultspace_app;
  ...
  IF NOT pg_catalog.has_function_privilege(
    ...
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'BOOTSTRAP_SESSION_RUNTIME_GRANT_INVALID';
  END IF;
  ...
  IF unexpected_acl_rows <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = 'P0001',
      MESSAGE = 'BOOTSTRAP_SESSION_RUNTIME_ACL_INVALID';
  END IF;
END
$$;
