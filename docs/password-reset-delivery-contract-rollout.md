...
The ordinary staging deployment workflow cannot perform the first version 1 activation. Until quick uncached health proves exact HMAC write mode, configured recovery, numeric delivery contract version 1, and the captured Azure web identity, its pre-mutation boundary deliberately requires the currently serving web, worker, and configured password-reset reconciler images to declare version 1 and share one source revision. This prevents a normal deployment or automatic rollback from crossing the compatibility boundary with a historical image.

After HMAC steady state is proven, automatic recovery may deliberately retain newer verified contract-v1 consumers while restoring an older contract-v1 web release. Subsequent deployment validates each captured web, worker, and configured reconciler artifact independently by immutable digest, full source-revision label, tag or digest binding, and delivery contract version instead of requiring one shared source revision. The captured public web still must match the sole active Azure revision, 100 percent traffic, image source release, and quick uncached health identity. Missing or ambiguous HMAC recovery evidence falls back to the strict same-source first-activation rule.
