...
- **Per-resource pages:** `/rooms/[roomId]` is now in the E2E suite (commit `fa97669`) — uses `/api/rooms` to resolve the first seeded room then runs axe. Still needed: `/rooms/[roomId]/settings`, `/rooms/[roomId]/audit`, `/rooms/[roomId]/trash`, `/rooms/[roomId]/analytics`, room detail tabs, the document viewer iframe, and the public viewer link landing flow.
