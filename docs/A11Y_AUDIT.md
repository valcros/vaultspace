...
- **Per-resource pages:** `/rooms/[roomId]` and sub-pages (`/settings`, `/audit`, `/trash`, `/analytics`) are now in the E2E suite (commits `fa97669`, `d3d6b7c`). Still needed: room detail tabs, the document viewer iframe, and the public viewer link landing flow.
...
- ~~**Wire `tests/e2e/a11y.test.ts` into CI.**~~ Done. CI `e2e` job added in `.github/workflows/ci.yml` (commit `7a294ea`) — seeds a standalone Postgres/Redis, runs `dev:standalone`, and executes `tests/e2e/a11y.test.ts` + `tests/e2e/security.test.ts` on every push/PR.
