...
const candidates = [
  ...
  { id: 'keep-REDACTED', name: 'Brightside', slug: 'REDACTED', _count: { users: 1 } },
...
describe('POST /api/sysop/organizations/bulk-disable', () => {
  beforeEach(() => {
    ...
    // keep-list resolves REDACTED by slug; operator membership adds op-org.
    mockOrgFindMany.mockImplementation((args: { where?: { slug?: unknown } }) => {
      if (args?.where && 'slug' in args.where) {
        return Promise.resolve([{ id: 'keep-REDACTED' }]);
      }
  ...
  it('dry-run lists only real junk; keep-list + operator org + multi-user EXCLUDED; no writes', async () => {
    ...
    expect(ids).not.toContain('keep-REDACTED');
  ...
  it('execute disables only confirmIds ∩ eligible; skips the rest', async () => {
    // Operator submits junk-1 (eligible), keep-REDACTED (NOT eligible), and a stale id.
    const res = await POST(
      makeRequest({ dryRun: false, confirmIds: ['junk-1', 'keep-REDACTED', 'stale-id'] })
    );
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.disabled).toEqual(['junk-1']);
    expect(body.skipped).toEqual(expect.arrayContaining(['keep-REDACTED', 'stale-id']));
