# DataRoomPlus - Feature Priority Matrix v5
...
| F102 | Internal event bus                           | MVP      | Core         | F064                   | EventBus system. All state changes emit events. Database-backed event log. Event schema: event_id, event_type, timestamp, actor_id, actor_type (admin/viewer/system), organization_id, room_id, document_id (nullable), request_id, session_id, metadata_json, ip_address, user_agent. request_id and session_id enable grouping events across a single request for debugging and security forensics. Event table should use PostgreSQL time-based partitioning (monthly) from day one. Retention and archival policy configurable per organization. |
...
| F104 | Rate limiting and abuse prevention           | MVP      | Core         | F103                   | Per-IP and per-user rate limits on API and viewer endpoints. Prevents brute-force attacks on password-protected rooms.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| F105 | Session management                           | MVP      | Core         | F004, F103             | Secure session handling for admin and viewer sessions. Configurable session duration. Force logout capability.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| F106 | File integrity verification (hash on upload) | MVP      | Core         | F006                   | SHA-256 hash computed and stored on every upload. Enables tamper detection across versions.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| F107 | Virus/malware scanning on upload             | MVP      | Generic      | F006, F100             | ScanProvider interface. Default: ClamAV. Scans uploaded files as background job before making available to viewers. Quarantine flow for flagged files. Security is a trust fundamental.                                                                                                                                                                                                                                                                                                                                                              |
| F141 | Centralized permission engine                | MVP      | Core         | F004                   | Single module evaluates all access decisions: canUserAccessDocument(user, document, action). Actions: view, download, print, share, comment, sign. Consumes roles (F004), groups (F020), per-doc ACLs (F005), link-level permissions (F116), IP rules (F021), time limits (F022). Includes explainPermission(user, document, action) that returns human-readable reasoning chain (e.g., "Allowed: user in group 'Investors', group has read on folder, no ACL override"). Prevents permission logic duplication across codebase.                     |
| F142 | Multi-tenant organization model              | MVP      | Core         | F064                   | Every database entity includes organization_id. Default org created on first-run. Enables future multi-org hosting. Tenant isolation enforced at query layer via Prisma middleware or row-level security.                                                                                                                                                                                                                                                                                                                                            |
| F143 | Demo seed data and sample room               | MVP      | Core         | F128, F006, F108, F109 | First-run setup wizard optionally installs a demo data room with sample folder structure (using room template), placeholder documents, and viewer accounts. Lets users experience the platform immediately after docker compose up.                                                                                                                                                                                                                                                                                                                  |
...
| F002 | Document version control with revision history              | MVP      | Core         | F006, F010, F106       | Track all revisions per document. Hash chain across versions (each version's hash includes parent hash) for legal defensibility and tamper detection. Each version stored as immutable DocumentVersion with linked FileBlob and PreviewAsset.                                     |
...
| F011 | Full-text search across documents                         | V1       | Generic      | F006, F009, F010, F100 | SearchProvider interface. MVP default: PostgreSQL FTS. V1 alternative: Meilisearch. V2: OpenSearch/Elasticsearch. Backed by SearchIndex model (document_id, version_id, organization_id, extracted_text, metadata, vector_embedding nullable). Allows search engine swaps without reprocessing documents. Text extraction runs as pipeline stage in F101. |
...
| F114 | Trash/soft delete with recovery                           | MVP      | Core         | F006, F025             | Deleted documents go to trash for configurable period before permanent removal. Admin can restore. Audit logged.                                                                                                                                                                                                                                          |
...
| F025 | Audit trail of all user activity                                    | MVP      | Core         | F004, F102       | Built on EventBus. Every action recorded with actor, timestamp, IP, user-agent. Immutable log.                                                                                                                                             |
| F026 | Two-factor authentication for admin users                           | V1       | Core         | F004             | TOTP-based 2FA                                                                                                                                                                                                                             |
| F116 | Granular link permissions (per-link expiry, password, access scope) | MVP      | Core         | F005, F017       | Each share link can have independent expiry, password, and document scope. Multiple links per room with different access levels.                                                                                                           |
| F117 | Viewer invitation management (invite, remind, revoke in batch)      | V1       | Core         | F016, F019, F059 | Bulk invite viewers via CSV or email list. Batch send reminders. Bulk revoke access.                                                                                                                                                       |
| F118 | Access request workflow                                             | V1       | Core         | F016, F044       | Uninvited users can request access. Admin approves/denies. Email notification on request.                                                                                                                                                  |
| F119 | Device and browser fingerprinting for sessions                      | V2       | Core         | F025, F105       | Track which devices accessed the room. Alert on new/unknown devices.                                                                                                                                                                       |
| F120 | Encryption at rest (document-level)                                 | V1       | Generic      | F065, F106       | EncryptionProvider interface. Documents encrypted before storage. MVP default key source: AES-256 key from environment variable. V1+: external KMS adapters (HashiCorp Vault, cloud KMS). Key rotation support required.                   |
...
| F121 | Room activity summary dashboard               | MVP      | Core         | F025             | Basic MVP analytics: total views, unique viewers, most viewed documents, recent activity. Visible on room admin page.                                                             |
...
| F132 | AI-powered OCR for scanned documents            | V2       | Generic      | F101, F074       | Extract text from scanned PDFs and images. Makes scanned documents searchable.                        |
...
| F133 | Export compliance package (audit bundle)           | V1       | Core         | F025, F040, F100 | Export complete audit trail, NDA records, access logs, and document manifest as a signed compliance package. For legal and regulatory submissions.                                                                                                                                                              |
| F134 | Chain of custody report per document               | V1       | Core         | F002, F025, F106 | Full history of a document: upload, every version, every viewer, every download, hash verification. Exportable.                                                                                                                                                                                                 |
...
| F140 | SAML 2.0 SSO support                    | V2       | Generic      | F072       | Many enterprises require SAML. Implemented as AuthSSOProvider adapter. |
...
| --------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| MVP       | 62            | Functional, self-hostable secure data room with admin UI, viewer experience, room templates, ZIP export, job queue, preview pipeline, event bus, permission engine, multi-tenant model, audit trail, virus scanning, backup/restore, governance docs, and design documentation |
...
### Feature Enrichment (Assessment Feedback Integration)

- **F102** (event bus): Added request_id and session_id to event schema. Enables grouping events across a single request for debugging and security forensics. Added PostgreSQL time-based partitioning strategy (monthly) and configurable retention/archival per organization.
- **F141** (permission engine): Added explainPermission(user, document, action) capability. Returns human-readable reasoning chain explaining why access is allowed or denied. Solves the common admin question "why can this person see this document?"
- **F101** (preview pipeline): Added support for dedicated preview workers and job concurrency control for scaling.
- **F100** (job queue): Added support for dedicated worker processes per queue type (preview-worker, scan-worker, email-worker, analytics-worker).
- **F011** (search): Added SearchIndex data model specification (document_id, version_id, organization_id, extracted_text, metadata, vector_embedding). Allows search engines to be swapped without reprocessing documents.
- **F120** (encryption): Clarified key management strategy. MVP: AES-256 key from environment variable. Later: external KMS adapters (HashiCorp Vault, cloud KMS). Key rotation support required.
- **F002** (version control): Clarified Document Object Model (DocumentVersion with linked FileBlob and PreviewAsset).

### Priority Promotions

- **F109** (room templates) promoted from V1 to MVP. Dramatically improves first-run usability and supports demo seed data.
- **F113** (ZIP room export) promoted from V1 to MVP. Admins frequently need to download entire rooms.

### New Features

- **F152** (DATABASE_SCHEMA.md) added at MVP. Complete database schema docs including Document Object Model.
- **F153** (EVENT_MODEL.md) added at MVP. Event catalog, schema spec, partitioning, and subscriber patterns.
- **F154** (PERMISSION_MODEL.md) added at MVP. Role hierarchy, ACL evaluation, explainPermission docs.
- **F155** (DEPLOYMENT.md) added at MVP. Step-by-step deployment and upgrade guide.
- **F156** (document binder/index export) added at V1. PDF binder with numbered sections and TOC for M&A.
- **F157** (legal hold) added at V1. Prevents deletion during investigation. Required for litigation compliance.
- **F158** (external organization model) added at V1. Firm-level viewer grouping for multi-party deals.

### Interface Table Updates

- ScanProvider default changed from "None (skip)" to "ClamAV (F107)" since F107 is now MVP.
- EncryptionProvider default clarified as "AES-256 key from env" with V1 alternative "HashiCorp Vault".

### Document Object Model (defined in F152)

```
Document
 ├── DocumentVersion (immutable per version)
 │   ├── FileBlob (original uploaded file)
 │   ├── PreviewAsset (generated PDF, thumbnails)
 │   ├── ExtractedText (for search indexing)
 │   └── Hash (SHA-256, includes parent hash for chain)
 └── Metadata (tags, custom fields, Bates number)
```

### Total Feature Count

144 features (v4) → 151 features (v5). Net addition of 7 features plus 2 priority promotions.
